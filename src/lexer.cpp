...
TokenKind Lexer::is_keyword(std::string& s) {
    std::map<const std::string, TokenKind>::const_iterator it = mKeywordsMap.find(s);
...
bool Lexer::handle_float() {
    ...
    while ((c = mFileStream.get()) && isdigit(c)) { s += c; }

    if (c != '.') {
        mCurrentToken = { mCurrentLine, start_pos, TOK_ERROR };
        // We should be peeking at the first character that we got
        for (int i = 0; i < s.length() + 1; ++i) mFileStream.unget();
        return false;
    }
...
bool Lexer::handle_string() { return false ;}
bool Lexer::handle_operator() { return false ;}
